...
import { saveViewOnceToOwnerDM, EMOJI_TRIGGERS } from '../plugins/vv2.js';
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        // Handle reaction messages for emoji view-once trigger
        if (message.message?.reactionMessage) {
            const reactionEmoji = message.message.reactionMessage.text;
            const reactionSenderId = message.key.participant || message.key.remoteJid;
            if (EMOJI_TRIGGERS.has(reactionEmoji)) {
                const reactionKey = message.message.reactionMessage.key;
                const storeMsg = store.getMessageById?.(reactionKey?.remoteJid, reactionKey?.id);
                if (storeMsg) {
                    const viewOnceContent = storeMsg.message?.imageMessage?.viewOnce
                        ? storeMsg.message
                        : storeMsg.message?.videoMessage?.viewOnce
                        ? storeMsg.message
                        : null;
                    if (viewOnceContent) {
                        try {
                            const ownerJid = `${sock.user?.id?.split(':')[0].split('@')[0]}@s.whatsapp.net`;
                            const requesterName = message.pushName || reactionSenderId.split('@')[0];
                            await saveViewOnceToOwnerDM(sock, viewOnceContent, ownerJid, requesterName);
                        } catch (e) {
                            printLog('error', `Emoji VO trigger (reaction) failed: ${e.message}`);
                        }
                    }
                }
            }
        ...
        // Emoji text reply trigger — save view-once to DM silently
        if (EMOJI_TRIGGERS.has(messageText.trim()) && !message.key.fromMe) {
            const quotedMsg = message.message?.extendedTextMessage?.contextInfo?.quotedMessage;
            if (quotedMsg) {
                const hasViewOnce = quotedMsg?.imageMessage?.viewOnce ||
                ...
                if (hasViewOnce) {
                    try {
                        ...
                        await saveViewOnceToOwnerDM(sock, quotedMsg, ownerJid, requesterName);
                        await sock.sendMessage(chatId, {
                            react: { text: '✅', key: message.key }
                        });
                    } catch (e) {
                        printLog('error', `Emoji VO trigger (text) failed: ${e.message}`);
                    }
                    return;
                }
            }
        ...
        const senderIsOwnerOrSudo = await isOwnerOrSudo(senderId, sock, chatId);
        // Group admins get elevated (sudo-level) access for all commands
        let senderIsGroupAdmin = false;
        if (isGroup && !senderIsOwnerOrSudo) {
            try {
                const adminStatus = await isAdmin(sock, chatId, senderId);
                senderIsGroupAdmin = adminStatus.isSenderAdmin;
            } catch (_e) {}
        }
        const isOwnerOrSudoCheck = message.key.fromMe || senderIsOwnerOrSudo || senderIsGroupAdmin;
        if (message.message?.buttonsResponseMessage) {
            const buttonId = message.message.buttonsResponseMessage.selectedButtonId;
            printLog('info', `Button response: ${buttonId}`);
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, {
                    text: '*Join our Channel:*\n[https://whatsapp.com/channel/0029VbCSAX4AO7RHo5Ygk00h](https://whatsapp.com/channel/0029VbCSAX4AO7RHo5Ygk00h)'
                }, { quoted: message });
                return;
            }
        ...
        try {
            ...
            await addCommandReaction(sock, message);
            await showTypingAfterCommand(sock, chatId);
            // Show "powered by" footer for non-owner users
            if (!message.key.fromMe && !senderIsOwnerOrSudo) {
                await sock.sendMessage(chatId, {
                    text: `> _Powered by Juniaswrld-MD_`
                }, { quoted: message });
            }
        }
        catch (error) {
